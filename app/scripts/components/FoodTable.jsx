	...
	render: function () {
		...
		if (this.state.ready) {
				...
				return (<tr key={'food-' + i}>
					...
					<td>{d.tipo}</td>
					{
						config.meses.map(function (mes) {
							return (
								<td className={(d[mes[1]] === 'Regular' ? 'regular' : (d[mes[1]] === 'Forte' ? 'strong' : 'weak'))}>{d[mes[1]]}</td>);
						})
					}
				</tr>);
			});
			...
			html = (
				...
				<table className="footable table table-food" ref="foodTable" data-page-size="15" data-filter="#filter">
					<thead>
					<tr>
